...
module.exports.createJudge = async (req,res)=>{
  try {
    ...
    const existingAdmin = await InnovvietJudge.findOne({ role: 'HackOnXJudge' });
    if (existingAdmin) {
...
module.exports.judgeLogin = async (req,res)=>{
  try {
    ...
    console.log("Judge login attempt with password:", password ? password : "No password provided");
...
module.exports.submitRating = async (req, res) => {
  try {
    ...
    const { innovation, technicality, presentation, feasibility, impact, comments } = req.body;
    
    // Validate input
    if (!innovation || !technicality || !presentation || !feasibility || !impact) {
      return res.status(400).json({
        success: false,
    ...
    const totalScore = (innovation + technicality + presentation + feasibility + impact) / 5;
    ...
    if (participant.ratings.length > 0) {
      const sum = participant.ratings.reduce((acc, rating) => acc + rating.totalScore, 0);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        ratings: participant.ratings.find(rating => rating.judgeId.toString() === judgeId.toString()),
...
module.exports.getTeamRatings = async (req, res) => {
  try {
    ...
    const participant = await HackathonParticipant.findById(teamId)
      .populate('ratings.judgeId', 'email'); // Populate judge details if needed
...
module.exports.getLeaderboard = async (req, res) => {
  try {
    ...
    const ratedParticipants = participants.filter(p => p.ratings && p.ratings.length > 0);
